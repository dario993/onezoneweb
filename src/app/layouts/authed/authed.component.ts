...
@Component({
  selector: 'layout-authed',
  templateUrl: './authed.component.html',
  styleUrls: ['./authed.component.scss'],
  imports: [CommonModule, RouterOutlet, I18nPipe],
  standalone: true,
})
export class LayoutAuthedComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public openFooterLink(link: string): void {
    switch (link) {
      ...
      case 'faq': {
        this.navigator.navigateTo(
          statics.LinkOneZoneFAQ[this.i18n.currentLanguage]
        );
        break;
      }
      case 'home': {
      ...
      case 'chat': {
        this.navigator.navigateTo('chat');
        break;
      }
    }
  }
